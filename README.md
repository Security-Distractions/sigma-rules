# Sigma rules — Security Distractions
...
bytecode was deliberately obfuscated before packaging — common in commodity Python malware, rare
in software an organisation buys or builds.
...
`.php.exe`, `.dat.exe` and `.lock.exe` — but it is published as untested and should be treated
that way. SigmaHQ already has `proc_creation_win_susp_double_extension.yml` (`1cdd9a09`, stable)
...
Corrections welcome, particularly false positives found in a real estate — that is the feedback
